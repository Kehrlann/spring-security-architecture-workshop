...
class AuthorizationApplicationTests {
	...
	class AdminPage {
		...
		void valid() {
			mvc.get().uri("/admin").exchange().assertThat().hasStatus(HttpStatus.OK);
		}

		@Test
		@WithMockUser(username = "test-user-with-a")
		void missingRole() {
			mvc.get().uri("/admin").exchange().assertThat().hasStatus(HttpStatus.FOUND).hasRedirectedUrl("/private");
		}
	...
	class OAuthPage {
		...
		@Test
		@WithMockUser("alice")
		void usernamePasswordUser() {
			mvc.get().uri("/oauth").exchange().assertThat().hasStatus(HttpStatus.FOUND).hasRedirectedUrl("/private");
		}
		...
		void oauthUser() {
			mvc.get()
				.uri("/oauth")
				.with(oidcLogin().idToken(token -> token.claim("email", "test-user@example.com")))
				.exchange()
				.assertThat()
				.hasStatus(HttpStatus.OK)
				.bodyText()
